...
function hasEmailCredentials() {
    const user = process.env.SMTP_USER || process.env.EMAIL_USERNAME || process.env.EMAIL_USER || '';
...
const sendEmail = asyncHandler(async (data) => {
    if (!hasEmailCredentials()) {
        throw new Error('Email credentials are not configured. Set SMTP_USER/SMTP_PASS or EMAIL_USERNAME/EMAIL_PASSWORD.');
    }
    ...
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            user: process.env.SMTP_USER || process.env.EMAIL_USERNAME || process.env.EMAIL_USER,
...
app.use((err, req, res, next) => {
    if (err?.type === 'entity.too.large') {
        return res.status(413).json({ error: 'Request payload is too large. Please use a smaller image or file.' });
    }
...
function setSessionCookie(res, sessionId, expiresAt) {
    ...
    res.append('Set-Cookie', `${sessionCookieName}=${encodeURIComponent(sessionId)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${maxAge}${secure}`);
}

function setAuthTokenCookie(res, token, expiresAt) {
    const maxAge = Math.max(0, Math.floor((expiresAt - Date.now()) / 1000));
    const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
    res.append('Set-Cookie', `${authTokenCookieName}=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${maxAge}${secure}`);
}

function clearSessionCookie(res) {
    res.append('Set-Cookie', `${sessionCookieName}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0`);
    res.append('Set-Cookie', `${authTokenCookieName}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0`);
}
...
async function getSessionUser(req) {
    ...
    if (authToken) {
        try {
            ...
            const user = await User.findOne({ email: String(decoded.sub || '').toLowerCase() }, '-password');
            if (user) {
                return { user, session: null, tokenExpiresAt: decoded.exp ? decoded.exp * 1000 : null };
            }
        } catch (error) {
...
async function sendWithConfiguredMailProvider(email, otp) {
    ...
    const hasGmailConfig = Boolean(process.env.EMAIL_USERNAME && process.env.EMAIL_PASSWORD);
...
async function sendWithSmtp(email, otp) {
    ...
    return { provider: 'smtp', messageId: info.messageId || null, preview: nodemailer.getTestMessageUrl(info) };
}
...
app.post('/api/auth/send-otp', async (req, res) => {
    ...
    pending.set(normalizedEmail, { otp, expiresAt, user: { name, email: normalizedEmail, password, role, gender } });
    try {
        const configuredMail = await sendWithConfiguredMailProvider(normalizedEmail, otp);
        if (configuredMail) {
            return res.json({
                ok: true,
...
app.post('/api/auth/send-login-otp', asyncHandler(async (req, res) => {
    ...
    pending.set(email.toLowerCase(), { otp, expiresAt, user: user.toObject(), login: true });
    try {
...
app.post('/api/auth/verify-login-otp', asyncHandler(async (req, res) => {
    const { email, otp } = req.body;
    if (!email || !otp) return res.status(400).json({ error: 'Missing fields' });

    const entry = pending.get(email.toLowerCase());
    if (!entry || !entry.login) return res.status(400).json({ error: 'No pending login OTP for this email' });
    if (Date.now() > entry.expiresAt) {
        pending.delete(email.toLowerCase());
        return res.status(400).json({ error: 'OTP expired' });
    }
    if (entry.otp !== otp.toString()) return res.status(400).json({ error: 'Invalid OTP' });
    ...
    await User.updateOne({ email: user.email }, { sessionExpiresAt: expiresAt });
...
app.post('/api/auth/forgot-password', asyncHandler(async (req, res) => {
    ...
    if (String(user.name).trim().toLowerCase() !== String(name).trim().toLowerCase()) {
        return res.status(400).json({ error: 'The provided name does not match our records' });
    }

    if (role && user.role !== role) {
        return res.status(400).json({ error: 'The provided role does not match our records' });
    }
    ...
    try {
        const configuredMail = await sendWithConfiguredMailProvider(normalizedEmail, otp);
        if (configuredMail) {
            return res.json({ ok: true, message: 'A reset code has been sent to your email.', provider: configuredMail.provider });
        }

        const previewUrl = await sendWithEthereal(normalizedEmail, otp);
        return res.json({ ok: true, message: 'A reset code has been sent to your email.', provider: 'ethereal', preview: previewUrl });
    } catch (err) {
...
app.post('/api/auth/reset-password', asyncHandler(async (req, res) => {
    ...
    if (!email || !otp || !password || !confirmPassword) {
        return res.status(400).json({ error: 'Email, OTP, password, and confirmation are required' });
    }
    ...
    await User.updateOne({ email: normalizedEmail }, { password: passwordHash });
    pending.delete(normalizedEmail);
...
app.post('/api/auth/verify-otp', asyncHandler(async (req, res) => {
    ...
    const normalizedEmail = email.toLowerCase();
    const entry = pending.get(normalizedEmail);
    if (!entry) return res.status(400).json({ error: 'No pending registration for this email' });
    if (Date.now() > entry.expiresAt) {
        pending.delete(normalizedEmail);
        return res.status(400).json({ error: 'OTP expired' });
    }
    ...
    const expiresAt = await createSession(res, newUser.email, newUser.toObject());
...
app.post('/api/auth/login', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const previewUrl = await sendWithEthereal(email, otp);
        // console.log('Email preview URL:', previewUrl);
        return res.json({ ok: true, twoFactor: true, message: 'OTP sent to your email. Please enter it to complete login.', provider: 'ethereal', preview: previewUrl });
    } catch (err) {
...
app.post('/api/auth/session/extend', asyncHandler(async (req, res) => {
    const sessionData = await getSessionUser(req);
    if (!sessionData) {
        clearSessionCookie(res);
        return res.status(401).json({ ok: false, error: 'Not authenticated' });
    ...
    setAuthTokenCookie(res, token, expiresAt);
    await User.updateOne({ email: sessionData.user.email }, { sessionExpiresAt: expiresAt });
    res.json({ ok: true, user: { ...sanitizeUser(sessionData.user), sessionExpiresAt: expiresAt } });
}));
...
app.put('/api/users/:email', asyncHandler(async (req, res) => {
    ...
    const user = await User.findOneAndUpdate({ email: req.params.email.toLowerCase() }, updates, {
        new: true,
        projection: '-password',
    });
    if (!user) return res.status(404).json({ error: 'User not found' });
...
app.post('/api/interviews/:sessionId/upload', asyncHandler(async (req, res) => {
    ...
    const safeName = String(filename || `${req.params.sessionId}-${Date.now()}.webm`).replace(/[^a-zA-Z0-9-_\.]/g, '_');
